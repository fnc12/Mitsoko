...
void sha256_print(unsigned char *digest,size_t len){
    size_t i;
    for(i=0;i<len;++i){
        unsigned char c;
...
int sha256_file(const char *filename,uint8 digest[32]){
    ...
    if(file){
        size_t bytesRead=0,portionSize=2048;
        char *data=new char[portionSize];
        if(data){
            bool endOfFile=false;
            do{
                bytesRead=fread(data,1,portionSize,file);
                if(bytesRead<portionSize){
                    endOfFile=true;
                }
                if(bytesRead){
                    sha256_update(&ctx,(unsigned char*)data,bytesRead);
                }
            }while(!endOfFile);
